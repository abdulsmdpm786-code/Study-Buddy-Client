...
function EditModal({ onClose, data }) {
  ...
  const [title, setTitle] = useState(data.subTitle);
  const [content, setContent] = useState(data.content);
  const [mainTitle, setMainTitle] = useState(data.mainTitle);
  const [type, setType] = useState(data.title.type)
  ...
  const handleSubmit = async () => {
    ...
    try {
      const addResponse = await AXIOS_API.patch(
        `/api/v1/course/content/${data._id}/edit`,
        updateData,
      );
