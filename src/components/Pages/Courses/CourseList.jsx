...
const isAdmin = user?.role === "admin";
  ...
  const handleDelete = async (id)=>{
    ...
    try {
      const deleteResponse  = await AXIOS_API.delete(`/api/v1/course/deleteCourse/${id}`)
              ...
              {isAdmin && (
                <div className="absolute top-4 right-4 flex items-center gap-1.5 opacity-90 group-hover:opacity-100 transition-opacity">
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg bg-white/90 backdrop-blur-sm text-slate-700 hover:bg-red-50 hover:text-red-600 shadow-sm transition-all active:scale-95"
            ...
            <div className="p-5 flex flex-col flex-grow justify-between gap-4">
              <div className="space-y-2.5">
                ...
                <div className="flex items-baseline justify-between pt-1">
                  ...
                  {/* Duration Badge */}
                  {data.duration && (
                    <div className="flex items-center text-xs font-medium text-slate-500 bg-slate-50 px-2 py-1 rounded-md border
                     border-slate-100">
                      <svg
